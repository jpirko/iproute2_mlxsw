...
#define DL_OPT_HANDLE_REGION		BIT(18)
#define DL_OPT_REGION_SNAPSHOT_ID	BIT(19)
#define DL_OPT_REGION_ADDRESS		BIT(20)
#define DL_OPT_REGION_LENGTH		BIT(21)
				...
				uint64_t addr)
{
	...
	while (i < len) {
		if (!dl->json_output)
			if (!(align_val % 16))
				pr_out("%016"PRIx64" ", addr);

		align_val++;

		if (dl->json_output)
			jsonw_printf(dl->jw, "%02x", data[i]);
		else
			pr_out("%02x", data[i]);
